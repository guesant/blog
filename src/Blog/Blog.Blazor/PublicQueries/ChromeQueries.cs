...
{
    internal static SiteRow? Site(BlogPublicDbContext db) =>
        db
    ...
    internal static string? ContactEmail(BlogPublicDbContext db) =>
        db.SiteSettings.OrderBy(s => s.Id).Select(s => s.ContactEmail).FirstOrDefault();

    internal static ProfileRow? Profile(BlogPublicDbContext db, string locale) =>
        (
    ...
    internal static string? Trajectory(BlogPublicDbContext db, string locale) =>
        (
            from p in db.Profiles
            from t in db
                .ProfileTranslations.Where(t => t.ProfileId == p.Id && t.Locale == locale)
    ...
    internal static List<PageRow> Pages(BlogPublicDbContext db, string locale) =>
        (
            from p in db.Pages
            from t in db
    ...
    internal static List<ContactProfileRow> ContactProfiles(BlogPublicDbContext db, int siteId) =>
        db
    ...
    internal static string? CopyrightTemplate(BlogPublicDbContext db, int siteId, string locale) =>
        db
            .SiteSettingsTranslations.Where(t => t.SiteSettingsId == siteId && t.Locale == locale)
            .Select(t => t.CopyrightTemplate)
            .FirstOrDefault();

    internal static List<NavRow> NavItems(BlogPublicDbContext db, string locale) =>
        (
