...
const MAX_PARAMS = { memorySize: 128 * 1024, iterations: 10, parallelism: 4, hashLength: 64 };
...
async function solve(challenge) {
  if (challenge.version !== 2 || challenge.algorithm !== 'argon2id-aes256gcm') {
    throw new Error('Unsupported challenge version');
  }

  const { memorySize, iterations, parallelism, hashLength } = challenge.params;
  if (
    memorySize > MAX_PARAMS.memorySize ||
