...
const arrayFieldsByRoot: Partial<Record<ContentDocumentType, string[]>> = {
...
const richTextFieldsByRoot: Partial<Record<ContentDocumentType, string[]>> = {
  caseStudy: ['body'],
  project: ['body'],
  experiment: ['body'],
  writing: ['body'],
...
function parseRichText(value: unknown) {
  ...
  return {
    ...
    children: value
      .split(/\n\s*\n/)
      .filter(Boolean)
      .map((paragraph) => ({ type: 'p', children: [{ type: 'text', text: paragraph }] })),
  };
}

function withParsedRichText<T>(
  document: FieldLocalizedDocument<T>,
  root: ContentDocumentType,
...
const technologyRoots = new Set<ContentDocumentType>(['caseStudy', 'project', 'experiment']);
...
const collectionRoots: Record<ContentCollection, ContentDocumentType> = {
  cases: 'caseStudy',
  projects: 'project',
...
export async function getLocalizedPage<T>(slug: string, locale?: string): Promise<T> {
  ...
  const pageRoot = `${slug}Page` as ContentDocumentType;
