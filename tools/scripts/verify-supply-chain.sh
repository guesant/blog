...
if find src/Blog -path '*/Blog.Blazor.Stories.VRT' -prune -o -type d -name node_modules -print -quit 2>/dev/null | grep -q .; then
...
if grep -REn -i 'https?://[^" ]*(cdn|unpkg|jsdelivr)|curl[[:space:]].*\|[[:space:]]*(sh|bash)|wget[[:space:]].*\|[[:space:]]*(sh|bash)' \
    src/Blog/Blog.Blazor src/Blog/Blog.Blazor.Client src/Blog/Blog.Blazor.Core src/Blog/Blog.Blazor.UI .docker/compose.yaml 2>/dev/null; then
    fail "remote scripts or CDN assets are not allowed"
fi

for project in src/Blog/*/*.csproj; do
