...
public partial class PercentageCalculator
{
    ...
    private PercentageResult _ofResult = Portfolio.Blazor.Core.PercentageCalculator.Calculate(
        100,
        15
    );
    private PercentageWhatPercentResult _whatPctResult =
        Portfolio.Blazor.Core.PercentageCalculator.WhatPercentOf(30, 150);
    private PercentageOfWhatResult _ofWhatResult =
        Portfolio.Blazor.Core.PercentageCalculator.OfWhat(30, 20);
    private PercentageChangeResult _changeResult =
        Portfolio.Blazor.Core.PercentageCalculator.Change(80, 100);
    ...
    {
        _ofResult = Portfolio.Blazor.Core.PercentageCalculator.Calculate(
            Parse(_ofYText),
            Parse(_ofXText)
        );
        _whatPctResult = Portfolio.Blazor.Core.PercentageCalculator.WhatPercentOf(
            Parse(_whatPctXText),
            Parse(_whatPctYText)
        );
        _ofWhatResult = Portfolio.Blazor.Core.PercentageCalculator.OfWhat(
            Parse(_ofWhatXText),
            Parse(_ofWhatYText)
        );
        _changeResult = Portfolio.Blazor.Core.PercentageCalculator.Change(
