...
var triangle = Blog.Blazor.Core.TriangleCalculator.Calculate(3, 4, 5);
if (
    !triangle.IsValid
    || !triangle.Summary.Contains("área = 6", StringComparison.Ordinal)
    || Blog.Blazor.Core.TriangleCalculator.Calculate(1, 2, 3).IsValid
)
{
...
if (
    Blog.Blazor.Core.VigenereCipher.Transform("ATTACKATDAWN", "LEMON", false) != "LXFOPVEFRNHR"
    || Blog.Blazor.Core.VigenereCipher.Transform("LXFOPVEFRNHR", "LEMON", true) != "ATTACKATDAWN"
)
{
    throw new InvalidOperationException("Vigenere cipher check failed");
}

var utf8 = Blog.Blazor.Core.Utf8Inspector.Inspect("Olá 😀");
